...
class CLIPVAD(nn.Module):
    ...
    def encode_video(self, images, padding_mask, lengths):
        ...
        attn_padding_mask = padding_mask if self.use_padding_mask else None
        x, _ = self.temporal((images, attn_padding_mask))
        x = x.permute(1, 0, 2)
    ...
    def forward(self, visual, padding_mask, text, lengths):
        ...
        visual_features_proj = visual_features + self.mlp2(visual_features)
        visual_features_proj = self.dropout(visual_features_proj)
        logits1 = self.classifier(visual_features_proj)
        ...
        visual_attn = visual_attn / visual_attn.norm(
            dim=-1, keepdim=True
        ).clamp_min(1e-12)
